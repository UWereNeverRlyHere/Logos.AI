...
public class QdrantService : IVectorStorageService
{
	...
	public async Task RecreateCollectionAsync(CancellationToken ct = default)
	{
		if (await _client.CollectionExistsAsync(_collectionName, ct))
		{
			...
			_logger.LogWarning("Collection {Name} was deleted by request", _collectionName);
		}
	...
	public async Task UpsertChunksAsync(IEnumerable<VectorPointDto> points, CancellationToken ct = default)
	{
